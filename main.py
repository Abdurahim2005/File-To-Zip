...
waiting_for_user_id: dict = {}   # admin waiting states
...
# NEW state dicts
user_donating:       dict = {}   # uid -> True
user_contact_admin:  dict = {}   # uid -> True (waiting to send msg to admin)
admin_reply_to:      dict = {}   # ADMIN_ID -> target_uid
user_zip_naming:     dict = {}   # uid -> {"chat_id": ..., "default_name": ...}
_user_file_locks:    dict = {}   # uid -> asyncio.Lock  (race condition fix)
...
def get_user_limits(uid: int) -> tuple:
    ...
    return (r[0], r[1]) if r else (MAX_ZIPS_DAY, MAX_STORAGE)
...
def set_user_zip_limit(uid: int, limit: int):
    ...
    if existing:
        ...
        c.execute("INSERT INTO user_limits(telegram_id,max_zips_day,max_storage_bytes) VALUES(?,?,?)",
                  (uid, limit, MAX_STORAGE))
    c.commit(); db_sync()
...
def set_user_storage_limit(uid: int, storage_bytes: int):
    ...
    if existing:
        ...
        c.execute("INSERT INTO user_limits(telegram_id,max_zips_day,max_storage_bytes) VALUES(?,?,?)",
                  (uid, MAX_ZIPS_DAY, storage_bytes))
    c.commit(); db_sync()
...
# ════════════════════════════════════════════════════════════
#  ZIP YASASH TUGMASI — asks for name first
# ════════════════════════════════════════════════════════════
...
async def cb_zip_now(client, call):
    ...
    # Auto-zip with default name after 30s if no name given
...
# ════════════════════════════════════════════════════════════
#  CONTACT ADMIN — user side
# ════════════════════════════════════════════════════════════
async def _handle_contact_media(client, message: Message):
    """Handle photo/video sent when user is in contact-admin mode."""
    uid  = message.from_user.id
    lang = get_lang(uid) or "uz"
...
async def _handle_admin_reply_media(client, message: Message):
    """Handle photo/video sent when admin is replying to user."""
...
async def cmd_premium(client, message):
    ...
    lang = get_lang(uid) or "uz"
    await safe_delete(message)
    await client.send_message(message.chat.id, TEXTS[lang]["premium_text"],
                              parse_mode=enums.ParseMode.MARKDOWN)
...
async def on_text(client, message):
    ...
    # ── Cancel contact ──
    # (handled via callback, but also via text)
...
# ── Inline confirm/reject donation from notification ─────
...
async def adm_search(client, call):
    ...
    await call.message.reply("🔍 Foydalanuvchi ID sini yuboring:"); await call.answer()
...
async def adm_ban(client, call):
    ...
    await call.message.reply("⛔ Ban qilmoqchi bo'lgan foydalanuvchi *ID* sini yuboring:",
                             parse_mode=enums.ParseMode.MARKDOWN); await call.answer()
...
async def adm_channels(client, call):
    ...
    await call.message.reply(text, parse_mode=enums.ParseMode.MARKDOWN,
                             reply_markup=InlineKeyboardMarkup(btns)); await call.answer()
...
async def adm_confirm_don(client, call):
    ...
    await call.message.reply("✅ Tasdiqlash uchun Don ID sini yuboring:"); await call.answer()
...
async def adm_reject_don(client, call):
    ...
    await call.message.reply("❌ Bekor qilish uchun Don ID sini yuboring:"); await call.answer()
