...
user_download_task:  dict = {}   # (eski — o'chirildi, bo'sh saqlanadi)
...
# ════════════════════════════════════════════════════════════
#  FAYLLARNI PARALLEL YUKLAB OLISH
# ════════════════════════════════════════════════════════════
async def _dl_one(client, file_id: str, save_path: str, arcname: str):
    """Bitta fayl — (file_id, save_path, arcname, ok)"""
    try:
        await client.download_media(file_id, file_name=save_path)
        return (file_id, save_path, arcname, True)
    except Exception:
        return (file_id, save_path, arcname, False)


async def download_all_parallel(client, uid: int) -> list:
    """
    DB dagi barcha file_id larni bir vaqtda parallel yuklab oladi.
    Qaytaradi: [(save_path, arcname), ...] — muvaffaqiyatli yuklanganlar
    """
    records    = get_user_files(uid)
    udir       = user_dir(uid)
    seen_names : set  = set()
    tasks      : list = []

    for _, file_id, _, filename, _ in records:
        ...
        tasks.append(_dl_one(client, file_id, save_path, arcname))

    results    = await asyncio.gather(*tasks, return_exceptions=True)
    downloaded = []
    for res in results:
        if isinstance(res, tuple) and res[3]:   # ok=True
            downloaded.append((res[1], res[2]))
    return downloaded
...
async def create_and_send_zip(client, chat_id: int, uid: int,
    ...
    try:
        ...
        downloaded = await download_all_parallel(client, uid)
