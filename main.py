...
MAX_FILES      = 25                   # max fayl soni per ZIP
MAX_ZIPS_DAY   = 3                    # kunlik ZIP limiti
...
ZIP_SEMAPHORE: asyncio.Semaphore = None   # app.run() da yaratiladi
...
def _load_channels():
    ...
    rows = get_db().execute("SELECT chat_id, title FROM channels").fetchall()
    required_channels = {r[0]: r[1] for r in rows}

def add_channel(chat_id: int, title: str):
    c = get_db()
    c.execute("INSERT OR REPLACE INTO channels(chat_id,title) VALUES(?,?)", (chat_id, title))
    c.commit(); db_sync()
    required_channels[chat_id] = title
...
def get_global_stats() -> dict:
    ...
    return {
        "total_zips": total_zips,  "today_zips": today_zips,
        "total_mb":   total_mb,    "today_mb":   today_mb,
...
def make_zip_name(user) -> str:
    """Foydalanuvchi ismidan ZIP nomi yasash"""
    ...
    if sm is None:
        try:
            sent = await client.send_message(chat_id, text,
                       parse_mode=enums.ParseMode.MARKDOWN, reply_markup=mup)
...
def start_auto_zip(client, chat_id: int, uid: int,
                   delay: int = AUTO_ZIP_DELAY, user_obj=None):
    schedule_task(user_auto_zip, uid,
                  _auto_zip_runner(client, chat_id, uid, delay, user_obj))
...
# ════════════════════════════════════════════════════════════
#  ZIP YASASH TUGMASI  (ism so'ralmaydi — to'g'ridan ZIP)
# ════════════════════════════════════════════════════════════
...
async def cb_zip_now(client, call):
    ...
    # Hali yuklanayotgan fayllar bormi
...
async def cb_top_donors(client, call):
    ...
    await call.message.reply(
        TEXTS[lang]["top_donors"].format(list="\n".join(lines)),
        parse_mode=enums.ParseMode.MARKDOWN,
    )
    await call.answer()
    ...
    if uid == ADMIN_ID and uid in waiting_for_user_id:
        ...
        if action == "add_channel":
            try:
                ...
                add_channel(chat.id, chat.title or raw)
                await message.reply(
                    f"✅ Kanal qo'shildi: *{chat.title}* (`{chat.id}`)",
                    parse_mode=enums.ParseMode.MARKDOWN,
                )
            except Exception:
                await message.reply(
                    f"❌ Kanal topilmadi: `{raw}`\nBot kanalga admin bo'lganligini tekshiring.",
                    parse_mode=enums.ParseMode.MARKDOWN,
                )
            return
        ...
        if action == "ban":
            if not data:
                ...
                return
            ban_user(target_id)
            await message.reply(f"⛔ *Bloklandi:* {data[1]} (`{target_id}`)", parse_mode=enums.ParseMode.MARKDOWN)
            try:
            ...
            await message.reply(f"✅ *Blokdan chiqarildi:* {data[1]} (`{target_id}`)", parse_mode=enums.ParseMode.MARKDOWN)
        elif action == "info":
            if not data:
                await message.reply(f"❌ `{target_id}` topilmadi.", parse_mode=enums.ParseMode.MARKDOWN)
                return
...
async def adm_channels(client, call):
    ...
    text     = ("📢 *Majburiy kanallar:*\n\n" +
                "\n".join(f"• {t} (`{cid}`)" for cid, t in channels.items())) \
               if channels else "📢 Hozircha kanal qo'shilmagan."
    btns = [[InlineKeyboardButton(f"🗑 {t} o'chirish", callback_data=f"adm_rmchan_{cid}")]
            for cid, t in channels.items()]
...
async def adm_rmchan(client, call):
    try:
        ...
        title = required_channels.get(cid, str(cid))
        remove_channel(cid)
