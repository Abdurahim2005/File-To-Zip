...
async def cb_comp_all_select(client, call):
    level = int(call.data.split("_")[2])
    database.set_all_users_compression(level)
...
async def cb_donate_show_form(client, call):
    ...
    await call.message.reply(
        texts_mod.TEXTS[lang]["donate_text"],
        parse_mode=enums.ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton(texts_mod.TEXTS[lang]["donate_done_btn"], callback_data="donate_done")
        ]]),
    )
    await call.answer()
...
async def cb_donate_done(client, call):
    uid  = call.from_user.id
    lang = database.get_lang(uid) or "uz"
    state.user_donating[uid] = True
    await call.message.reply(texts_mod.TEXTS[lang]["donate_ask"], parse_mode=enums.ParseMode.MARKDOWN)
    await call.answer()
...
async def cb_reply_to_user(client, call):
    ...
    await call.message.reply(texts_mod.TEXTS["uz"]["admin_reply_ask"], parse_mode=enums.ParseMode.MARKDOWN)
    await call.answer()
...
async def cb_inline_confirm_don(client, call):
    ...
    database.confirm_donation(don_id)
    pend = database.get_db().execute(
...
async def cb_inline_reject_don(client, call):
    ...
    database.reject_donation(don_id)
    if pend:
        try:
            await client.send_message(
                pend[0],
...
async def cb_zip_now(client, call):
    ...
    await cancel_task(state.user_debounce, uid)
    await call.answer()

    default_name = make_zip_name(user)
    state.user_zip_naming[uid] = {"chat_id": call.message.chat.id, "default_name": default_name}

    sm = state.user_status_msg.get(uid)
    ...
    async def _zip_name_timeout():
        ...
        if uid in state.user_zip_naming:
            ...
            if info and file_count(uid) > 0:
                sm2 = state.user_status_msg.pop(uid, None)
                await safe_delete(sm2)
                await create_and_send_zip(client, info["chat_id"], uid, info["default_name"])

    schedule_task(state.user_auto_zip, uid, _zip_name_timeout())
